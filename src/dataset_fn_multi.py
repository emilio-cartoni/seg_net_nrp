...
def get_multi_dataloaders(mode, data_dir, batch_size, n_frames, shuffle,
                          drop_last, num_workers, remove_ground=True):
    
    # Dataset info
    train_path = os.path.join(data_dir, f'{mode}.hdf5')
    ...
    # Training dataloader
    dataset = Multi_Dataset(train_path,
                            n_frames,
                            n_classes,
                            augmentation,
                            remove_ground)
    ...
    # Return the dataloader and number of classes
