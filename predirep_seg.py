...
from keras.layers import Conv2D, UpSampling2D, MaxPooling2D, AveragePooling2D, Flatten, BatchNormalization, Conv2DTranspose, ReLU, Add
...
class PredNet(Recurrent):
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        default_output_modes = ['prediction', 'error', 'all', 'test', 'segmentation']
        layer_output_modes = [layer + str(n) for n in range(self.nb_layers) for layer in ['R', 'E', 'A', 'Ahat', 'Seg']]
    ...
    def compute_output_shape(self, input_shape):
        ...
        elif self.output_mode == 'test':
            out_shape = (2,)
        elif self.output_mode == 'segmentation':
            out_shape = (120, 160, 3)
        elif 'Seg' in self.output_mode:
            stack_str1 = 'R_stack_sizes'
            stack_str2 = 'stack_sizes'
            stack_mult1 = 1
            stack_mult2 = 2
            out_stack_size = stack_mult1 * getattr(self, stack_str1)[self.output_layer_num] + stack_mult2 * getattr(self, stack_str2)[self.output_layer_num]
    ...
    def build(self, input_shape):
        ...
        self.conv_layers = {c: [] for c in ['i', 'f', 'c', 'o', 'h', 'ahat', 'seg']}
        self.conv_trps_layers = {c: [] for c in ['seg']}
        ...
        for l in range(self.nb_layers):
            ...
            if l < self.nb_layers - 1:
                self.conv_trps_layers['seg'].append(Conv2DTranspose(filters=self.R_stack_sizes[l], kernel_size=3, strides=2, padding='same', output_padding=1, use_bias=False, data_format=self.data_format))

            act = 'relu' if l == 0 else self.LSTM_activation
        ...
        self.conv_layers['seg'].append(Conv2D(filters=3, kernel_size=1, use_bias=False, activation='softmax', data_format=self.data_format))
        ...
        for c in sorted(self.conv_trps_layers.keys()):
            for l in range(len(self.conv_trps_layers[c])):
                ds_factor = 2 ** l
                if c == 'seg':
                    nb_channels = self.R_stack_sizes[l+1]
                in_shape = (input_shape[0], nb_channels, nb_row // ds_factor, nb_col // ds_factor)
                if self.data_format == 'channels_last': in_shape = (in_shape[0], in_shape[2], in_shape[3], in_shape[1])
                with K.name_scope('layer_' + c + '_' + str(l)):
                    self.conv_trps_layers[c][l].build(in_shape)
                self.trainable_weights += self.conv_trps_layers[c][l].trainable_weights
    ...
    def step(self, a, states):
        ...
        c_tm1 = states[1*self.nb_layers:2*self.nb_layers]
        ...
        for l in range(self.nb_layers):
            _ahat = self.conv_layers['ahat'][l].call(r[l])
            if l == 0:
        ...
        for l in range(self.nb_layers):
            ...
            inputs = [r[l], e[l]]
            inputs = K.concatenate(inputs, axis=self.channel_axis)
            _r = self.conv_layers['h'][l].call(inputs)
            r_up.append(_r)
        ...
        x = self.batchn1(r[3])
        x = self.conv_trps_layers['seg'][2].call(x)
        x = self.relu(x)
        x = self.add([x, r[2]])
        x = self.batchn2(x)
        x = self.conv_trps_layers['seg'][1].call(x)
        x = self.relu(x)
        x = self.add([x, r[1]])
        x = self.batchn3(x)
        x = self.conv_trps_layers['seg'][0].call(x)
        x = self.relu(x)
        x = self.add([x, r[0]])
        x = self.batchn4(x)
        output = self.conv_layers['seg'][0].call(x)
